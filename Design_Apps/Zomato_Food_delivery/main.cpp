...
int main() {
    ...
    vector<Restaurent*> restaurantList = tomato->searchRestaurants("Delhi");
    ...
    tomato->addToCart(user, "P4");

    tomato->printUserCart(user);

    // User checkout the cart
    Order* order = tomato->checkoutScheduled(user, "Delivery", new UpiPaymentStrategy("1234567890"), "Tomorrow");
